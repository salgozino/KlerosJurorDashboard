...
from app.modules.subgraph import getArbitrableName, getCourtName
...
def disputesbyCourtGraph(disputes, language="en"):
    ...
    data.index = data.index.map(getCourtName)
    data = data.to_dict()
...
def disputesbyArbitratedGraph(disputes, language="en"):
    ...
    fig = go.Figure()
    df = pd.DataFrame(disputes)
    df.arbitrable = df.arbitrable.map(getArbitrableName)
